        ...
        <div class="row">
            <div class="col-md-12">
                <div class="d-flex justify-content-end mb-2 mr-3">
                    <a type="button" class="btn btn-primary btn-icon-text" href="{{ route('brand.create') }}"><i class="ti-plus btn-icon-prepend"></i>Add Brand
                    </a>
                </div>
            </div>
        </div>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex justify-content-between">
                        ...
                        <div class="">
                            <p class="text-right"> Total Record {{ $totalBrands }}</p>
                        </div>
                    </div>
                            ...
                            <tbody>
                                @foreach($brands as $key => $brand)
                                    <tr>
                                        <td>{{$key+1}}</td>
                                        <td>{{$brand->name}}</td>
                                        <td><label class="badge badge-{{$brand->status->meta['color']}}">{{$brand->status->value}}</label></td>
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow" style="">
                                                    ...
                                                    @can('user.delete')
                                                        <a class="dropdown-item" onclick='swal({
                                                                ...
                                                                function(){
                                                                document.getElementById("delete-job-{{ $brand->id }}").submit();
